        ...
        if (st->info->duration_count>1 && !st->r_frame_rate.num
            ...
            for (j= 0; j<MAX_STD_TIMEBASES; j++) {
                ...
                for (k= 0; k<2; k++) {
					...
					//debug for DTS2014073003900, these logs are so many
                    // if (error < 0.02)
...
int k3_find_stream_info(AVFormatContext *ic, int tryDecodeNumber)
{
	...
	//added according to ffmpeg 2.5.3 avformat_find_stream_info function begin
	...
	//added according to ffmpeg 2.5.3 avformat_find_stream_info function end
    if (ic->pb)
...
/*            if (!st->time_base.num)
    ...
    //for(;;) {
        ...
        for (i = 0; i < ic->nb_streams; i++) {
           ...
           //     st->codec_info_nb_frames < ic->max_ts_probe &&  //deleted by xwx271409 to revise the error timestamp of aac_audio for DTS2015042600246 at 2015.4.28
                (st->codec->codec_type == AVMEDIA_TYPE_VIDEO ||

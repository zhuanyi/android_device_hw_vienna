...
static int av_log_level = AV_LOG_ERROR;	////modified by xwx271409 to fix ffmpeg log level for DTS2015052100642
static int flags;
...
//start debug for DTS2014073003900, these logs are so many.by z00183533
void av_log_default_callback_use_ALog(void* ptr, int level, const char* fmt, va_list vl)
{
    ...
    //added by xwx271409 to set the level of log_print for DTS2015042600280 at 2015.4.28 begin
    if (level > av_log_level)
        return;
    //added by xwx271409 to set the level of log_print for DTS2015042600280 at 2015.4.28 end
    ...
    else if (level == AV_LOG_WARNING)  //added by xwx271409 to set the level of log_print for DTS2015042600280 at 2015.4.28 begin
    {
        ALOG(LOG_WARN, LOG_TAG,"%s" , logBuf);
    }
    else if (level == AV_LOG_VERBOSE)  //added by xwx271409 to fix ffmpeg log level for DTS2015052100642 begin
    {
        ALOG(LOG_VERBOSE, LOG_TAG,"%s" , logBuf);
    }
    else if (level > AV_LOG_VERBOSE)  //added by xwx271409 to fix ffmpeg log level for DTS2015052100642 end
    {
        ALOG(LOG_DEBUG, LOG_TAG,"%s" , logBuf);
    }   //added by xwx271409 to set the level of log_print for DTS2015042600280 at 2015.4.28 end
    else {
        ALOG(LOG_FATAL, LOG_TAG,"%s" , logBuf); //modified by xwx271409 to set the level of log_print for DTS2015042600280 at 2015.4.28
    }
}
//end debug for DTS2014073003900, these logs are so many.by z00183533
